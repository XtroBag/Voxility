...
CommandFolders.forEach(cmdfldr => {
   fs.readdir(`./src/commands/${cmdfldr}`, (err, data) => {
       ...
       data.forEach(specFile => {
           commandFiles[cmdfldr].push(specFile.substring(0, specFile.lastIndexOf('.')) || specFile);
       });
   })
})
    ...
    const MainPage = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: `Developer Team:`,
          value:
            `${client.Owner} Owner: ${config.Users.Owner.Name}
        ...
        },
          {
              name: `Support Server:`,
              value:
                  `${client.Invite} Invite: [Server Invite](${invite})`,
              inline: false,
          },
      )
      .setColor("#36393e")
    ...
    interaction.reply({ embeds: [MainPage], components: [SelectMenu], fetchReply: true }).then(async (msg) => {
      let filter = i => i.user.id === interaction.user.id;
      let collector = await msg.createMessageComponentCollector({ filter: filter });
      ...
      collector.on('collect', async (interaction) => {
        if (interaction.isSelectMenu()) {
          if (interaction.customId === "help_menu") {
            ...
            .setDescription(`>>> \`\`${commandFiles[directory].join("\`\` \`\`") || "No files in directory"}\`\``)
